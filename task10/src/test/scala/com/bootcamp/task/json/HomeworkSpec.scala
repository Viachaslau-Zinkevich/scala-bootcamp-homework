package com.bootcamp.task.json
...
object HomeworkSpec {
  ...
  implicit val localDateDecoder: Decoder[LocalDate] = Decoder.decodeLocalDateWithFormatter(DateTimeFormatter.BASIC_ISO_DATE)
  implicit val localDateEncoder: Encoder[LocalDate] = Encoder.encodeLocalDateWithFormatter(DateTimeFormatter.BASIC_ISO_DATE)

  implicit val teamTotalsConfig: Configuration = Configuration.default.copy(transformMemberNames = {
  ...
  implicit val teamTotalEncoder: Encoder[TeamTotals] = deriveConfiguredEncoder[TeamTotals]
